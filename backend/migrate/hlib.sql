...
BEGIN
    ...
    -- XÃ³a táº¥t cáº£ tÃ i liá»‡u trong collection
    DELETE FROM documents WHERE cid = collection_id;

    -- XÃ³a collection chÃ­nh nÃ³
...
BEGIN
    ...
    -- XÃ³a táº¥t cáº£ cÃ¡c section trong course
    DELETE FROM section WHERE course_id = course_id;

    -- XÃ³a course chÃ­nh nÃ³
...
FOR EACH ROW
BEGIN
    -- Cáº­p nháº­t ngÃ¢n sÃ¡ch cá»§a ngÆ°á»i dÃ¹ng sau khi Ä‘áº·t tÃ i liá»‡u
    UPDATE users
    SET budget = budget + (
